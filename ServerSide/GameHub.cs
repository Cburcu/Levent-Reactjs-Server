    ...
    public class GameHub : Hub
    {
        ...
        public async Task JoinGroup(string playerName)
        {
            ...
            if (waitingUserId == null)
            {
                ...
                await Clients.Client(waitingUserId).InvokeAsync("WaitingOpponent", "Rakip bekleniyor..");
            }
            else
            {
                ...
                await Clients.Client(waitingUserId).InvokeAsync("StartGame",
                    "Oyuna girildi. Rakibiniz " + playerName + ". Hamle sırası sizde!",
                    "turnOwner",
                    waitingUserName, playerName,
                    Game.LettersPoints);
                await Clients.Client(Context.ConnectionId).InvokeAsync("StartGame",
        ...
        public override async Task OnDisconnectedAsync(Exception exception)
        {
            ...
            {
                await Clients.Client(game.playerConnectionId).InvokeAsync("RestartGame", game.playerName, "Rakip oyundan ayrıldı. Lütfen oyunu yeniden başlatın!");
            }
            else
            {
                await Clients.Client(game.waitingPlayerConnectionId).InvokeAsync("RestartGame", game.waitingPlayerName, "Rakip oyundan ayrıldı. Lütfen oyunu yeniden başlatın!");
            }
        }
        ...
        public async Task Play(char letter, int xDimension, int yDimension)
        {
            ...
            try
            {
                ...
                await Clients.Client(opponentConnectionId)
                    .InvokeAsync("PlayOpponentLetter", "Hamle sırası sizde! Lütfen " + letter + " harfini kullanın. ", letter, "OpponentTurn");

                await Clients.Client(turnOwnerConnectionId)
                    .InvokeAsync("PlayOpponentLetter", game.GameInstance.GetOpponent().Username + " 'in hamle sırası!", letter, "TurnOwner");
            }
            catch (IncorrectLetterException exc)
            {
                await Clients.Client(turnOwnerConnectionId)
                    .InvokeAsync("IncorrectLetterException", exc.Message);
            }
            catch (GridCellException exc)
            {
                await Clients.Client(turnOwnerConnectionId)
                    .InvokeAsync("GridCellException", exc.Message);
            }
        }
        ...
        public async Task PlayOpponent(int opponentDimensionx, int opponentDimensiony)
        {
            ...
            try
            {
                ...
                if (game.GameInstance.GameIsOver())
                {
                    ...
                    await Clients.Client(turnOwnerConnectionId).InvokeAsync("GameIsOver", "OYUN BİTTİ!!!", new
                    {
                    ...
                    await Clients.Client(opponentConnectionId).InvokeAsync("GameIsOver", "OYUN BİTTİ!!!", new
                    {
                        winnerName = winner.User.Username,
                        winnerScore = winner.Score,
                        winnerMeaningfulWords = winner.MeaningfulWords,
                ...
                {
                    await Clients.Client(turnOwnerConnectionId)
                        .InvokeAsync("TurnOwnwer", "Hamle sırası sizde!", "TurnOwner");
                    await Clients.Client(opponentConnectionId)
                        .InvokeAsync("TurnOwnwer", game.GameInstance.TurnOwner.Username + " 'in hamle sırası!", "Opponent");
                }
            }
            catch (GridCellException e)
            {
                await Clients.Client(opponentConnectionId)
                    .InvokeAsync("GridCellException", e.Message);
            }
            catch (IncorrectLetterException e)
            {
                await Clients.Client(opponentConnectionId)
                    .InvokeAsync("IncorrectLetterException", e.Message);
            }
        }
    }
